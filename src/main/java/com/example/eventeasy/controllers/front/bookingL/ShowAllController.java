...
public class ShowAllController implements Initializable {
    ...
    public Parent makeBookingLModel(
        ...
        Parent parent = null;
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.FXML_FRONT_MODEL_BOOKING));
            parent = loader.load();

            HBox innerContainer = (HBox) ((AnchorPane) ((AnchorPane) parent).getChildren().get(0)).getChildren().get(0);
            ((Text) innerContainer.lookup("#prixText")).setText("Prix : " + bookingL.getPrix());
            ((Text) innerContainer.lookup("#dateDText")).setText("Date De Debut : " + bookingL.getDateD());
            ((Text) innerContainer.lookup("#dateFText")).setText("Date De Fin : " + bookingL.getDateF());

            LieuService lieuService = new LieuService();
            String nomLieu = lieuService.getLieuById(bookingL.getLieub_id()).getNom();
            ((Label) innerContainer.lookup("#labelLieuName")).setText("Nom Lieu : " + nomLieu);
    ...
    private void modifierBookingL(BookingL bookingL) {
        ...
        ManageController controller = MainWindowController.getInstance().loadManageInterface();
        controller.initData(bookingL.getLieub_id());
    }
