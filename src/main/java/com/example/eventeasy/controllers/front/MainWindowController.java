...
public class MainWindowController implements Initializable {
    ...
    public ManageController loadManageInterface() {
        ManageController controller = null;
        staticContent.getChildren().clear();
        if (getClass().getResource(Constants.FXML_FRONT_MANAGE_BOOKING_L) == null) {
            System.out.println("Could not load FXML check the path");
        } else {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.FXML_FRONT_MANAGE_BOOKING_L));
                Parent parent = loader.load();
                controller = loader.getController(); // Getting the controller
                AnchorPane.setTopAnchor(parent, 0.0);
                AnchorPane.setBottomAnchor(parent, 0.0);
                AnchorPane.setRightAnchor(parent, 0.0);
                AnchorPane.setLeftAnchor(parent, 0.0);
